...
namespace ca
{
	...
	*/
	...
	void kernel(
		...
		T* rule, 
		size_ptr neighbours, 
		size_ptr numberOfChangedElements)
	{
		//extern __shared__ T nextCaState[];


		dim3 indexes(0, 0, 0);
		indexes.x = threadIdx.x + blockIdx.x * blockDim.x;
		indexes.y = threadIdx.y + blockIdx.y * blockDim.y;

		if (indexes.x < caDimensions->x && indexes.y < caDimensions->y)
		{
			std::size_t index = DimensionConverter::_3to1(*caDimensions, indexes);
			if (oldCaState[index])
			{
				T v = newCaState[index];
				newCaState[index] &= ~(DeviceRule<T>::applyRule(oldCaState, caDimensions, rule, neighbours, indexes));
				if (v != newCaState[index])
				{
		...
		//T nextCaState[1024] = { 0 };

		//std::size_t caSize = caDimensions->x * caDimensions->y;
		////printf("hello world\n");
		////printf("%d\n", threadIdx.z);

		//for (std::size_t k = 0; k < 1; ++k)	// 195
		//{
		...
		size_ptr numberOfChangedElements)
	{
		...
		kernel<<<gridSize, blockSize/*, sharedMemSize*/>>>(caState, otherCaState, caDimensions, rule, neighbours, numberOfChangedElements);
